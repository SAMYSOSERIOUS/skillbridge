...
- Escape-route hops pass a **relatedness gate** (config `paths.relatedness_gate`): a hop is kept only if O*NET's own Related Occupations lists the target for the hop origin, or the target is within the origin's closest gap quantile (`close_gap_quantile`). This uses O*NET's expert adjacency judgments to keep multi-hop ladders realistic instead of letting the highest-wage occupation headline every route; kept/gated counts land in `meta.json`.
